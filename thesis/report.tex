		...
		The representational power of neural networks is well defined theoretically: in fact, any continuous function can be implemented in a three-layer net, given sufficient number of hidden units, proper nonlinearities, and weights \cite{hornik1989multilayer,kuurkova1992kolmogorov,lam2015prslides}.
			...
			\subsection{Network architectures}
				...
				\begin{figure}[t]
					...
					\label{fig:recurrent}
				\end{figure}
				Contrarily to feedforward networks, \emph{recurrent} neural networks have at least one \emph{feedback} loop. Feedback is present in a system when the output of an element influences (at least in part), the input applied to that same element. The presence of feedback loops has a deep impact on the learning capabilities of recurrent neural networks \cite{haykin2009neural}. They are general computers, much more powerful than simple feedforward networks; for example, they have the ability to predict element in sequ
				ences on the basis of their predecessors \cite{cleeremans1993mechanisms}.
				
				\subsubsection{Deep networks}
				A deep neural network is a neural network with multiple hidden layers. The main idea behind this is that the extra layers enable composition of features from lower layers, enabling them to model complex data at a higher level of abstraction \cite{bengio2009learning,schmidhuber2015deep}. 
